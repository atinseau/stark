import { type Tracer, type Span, type SpanContext, SpanStatusCode, context, trace } from "@opentelemetry/api";
import type { BasicTracerProvider } from "@opentelemetry/sdk-trace-base";

import type { AgentIdentity } from "../types/agent.types.ts";
import { createTracerProvider, type TracerProviderConfig } from "./create-tracer-provider.ts";
...
export class AgentTracer {
  ...
  constructor(
    ...
    if (this.enabled) {
      ...
      this.provider = config?.provider ?? createTracerProvider({
        ...config,
  ...
  startToolCall(attrs: ToolCallSpanAttributes): Span {
    ...
    if (!this.tracer || !parentSpan || parentSpan === NOOP_SPAN) return NOOP_SPAN;
  ...
  endToolCall(toolCallId: string, status?: string, exitCode?: number): void {
    ...
    span.setStatus(
      failed
        ? { code: SpanStatusCode.ERROR, message: `Tool ${status ?? "failed"} (exit ${exitCode ?? "?"})` }
        : { code: SpanStatusCode.OK },
    );
    ...
    return this.tracer.startSpan(
      "agent.permission",
      {
        attributes: {
          ...
          ...(attrs.toolCallTitle && { "permission.tool_call_title": attrs.toolCallTitle }),
        },
      },
      parentCtx,
    );
  ...
  endPermission(
    ...
    if (details?.optionId) span.setAttribute("permission.option_id", details.optionId);
    if (details?.optionName) span.setAttribute("permission.option_name", details.optionName);
    if (details?.reason) span.setAttribute("permission.denial_reason", details.reason);

    span.setStatus(
      outcome === "granted"
        ? { code: SpanStatusCode.OK }
        : { code: SpanStatusCode.ERROR, message: details?.reason ?? "Permission denied" },
    );
    span.end();
  }
    ...
    return this.tracer.startSpan(
      `agent.fs.${attrs.operation}`,
      {
        attributes: {
          ...
          ...(attrs.contentLength !== undefined && { "fs.content_length": attrs.contentLength }),
        },
      },
      parentCtx,
    );
   ...
   */
  endTerminal(span: Span, exitCode?: number | null, signal?: string | null): void {
    if (span === NOOP_SPAN) return;
    ...
    span.setStatus(
      failed
        ? { code: SpanStatusCode.ERROR, message: `Terminal exited with code ${exitCode}` }
        : { code: SpanStatusCode.OK },
    );
    span.end();
  }
  ...
  async flush(): Promise<void> {
    ...
    for (const [toolCallId, span] of this.activeToolSpans) {
      span.setStatus({ code: SpanStatusCode.ERROR, message: "Agent destroyed before tool call completed" });
      span.end();
      this.activeToolSpans.delete(toolCallId);
    }
    ...
    if (this.activePromptSpan && this.activePromptSpan !== NOOP_SPAN) {
      this.activePromptSpan.setStatus({ code: SpanStatusCode.ERROR, message: "Agent destroyed during prompt" });
