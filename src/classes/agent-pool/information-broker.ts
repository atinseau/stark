...
export class InformationBroker {
  ...
  private async evaluateBatch(
    ...
    try {
      // Reasoning models (e.g. gpt-5-nano) consume a large portion of
      // maxTokens on internal reasoning tokens. The previous 300–500
      // per-target budget was systematically too low, causing empty
      // content responses and cascading agent failures.
      const effectiveMaxTokens = delta.promptResultSummary
        ? 4096 * targetStates.length
        : 2048 * targetStates.length;
