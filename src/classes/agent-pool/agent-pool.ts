...
export class AgentPool extends EventEmitter {
  ...
  constructor(config: AgentPoolConfig) {
    ...
    this.conversations.register(
      ConversationRole.REFLECTION,
      reflectionSystemPrompt({}),
      modelOverrides[ConversationRole.REFLECTION],
    );
    ...
    this.conversations.setUsageCallback(
      (role, inputTokens, outputTokens, costUsd) => {
        const sourceMap: Record<string, UsageSource> = {
          ...
          [ConversationRole.REFLECTION]: "reflection",
        };
  ...
  private async executeSubtasks(
    ...
    // Normalize dependencies to avoid deadlocks from invalid IDs
    const subtaskIdSet = new Set(analysis.subtasks.map((s) => s.id));
    for (const subtask of analysis.subtasks) {
      const normalized = new Set<string>();

      // Prefer explicit dependencies if they exist and are valid
      for (const depId of subtask.dependencies) {
        if (depId !== subtask.id && subtaskIdSet.has(depId)) {
          normalized.add(depId);
        }
      }

      // Fallback: reconstruct from analysis.dependencies (from -> to)
      if (normalized.size === 0) {
        for (const dep of analysis.dependencies) {
          if (
            dep.to === subtask.id &&
            dep.from !== subtask.id &&
            subtaskIdSet.has(dep.from)
          ) {
            normalized.add(dep.from);
          }
        }
      }

      const newDependencies = [...normalized];
      if (newDependencies.length !== subtask.dependencies.length) {
        this.logger.warn(
          {
            subtaskId: subtask.id,
            originalDependencies: subtask.dependencies,
            normalizedDependencies: newDependencies,
          },
          "Normalized subtask dependencies to valid IDs",
        );
        (subtask as { dependencies: string[] }).dependencies = newDependencies;
      }
    }

    // Drop invalid dependency edges that reference missing subtasks
    if (analysis.dependencies.length > 0) {
      const filteredDeps = analysis.dependencies.filter(
        (d) => subtaskIdSet.has(d.from) && subtaskIdSet.has(d.to),
      );
      if (filteredDeps.length !== analysis.dependencies.length) {
        this.logger.warn(
          {
            originalCount: analysis.dependencies.length,
            filteredCount: filteredDeps.length,
          },
          "Filtered invalid dependency edges",
        );
        (analysis as { dependencies: TaskDependency[] }).dependencies = filteredDeps;
      }
    }
    ...
    while (remaining.size > 0) {
      ...
      if (readyIds.length === 0) {
        ...
        // If replan didn't happen or said continue/abort, attempt a one-time bypass:
        // run remaining subtasks without blocking constraints to avoid false deadlocks.
        if (!deadlockBypassUsed) {
          deadlockBypassUsed = true;
          this.logger.warn(
            { remaining: [...remaining] },
            "Deadlock bypass engaged — executing remaining subtasks without blocking dependencies",
          );
          for (const id of remaining) {
            blockingDeps.set(id, new Set());
          }
          continue;
        }

        // Fallback: mark remaining subtasks as failed due to deadlock
        for (const id of remaining) {
