...
export class OpenRouterClient {
  ...
  async chatJson<T>(
    ...
    for (let attempt = 0; attempt < effectiveMaxAttempts; attempt++) {
      let raw: string;
      try {
        raw = await this.chat(conversationMessages, jsonOptions);
      } catch (chatError) {
        // Reasoning models (e.g. gpt-5-nano) can sporadically return
        // empty content even with a generous maxTokens budget. Retry
        // instead of failing immediately.
        const errMsg = toErrorMessage(chatError);
        this.logger.warn(
          { attempt, error: errMsg },
          `Chat call failed (attempt ${attempt + 1}) — ${errMsg}, retrying`,
        );
        continue;
      }
      lastRaw = raw;
   ...
   *
   * Handles three content shapes returned by the OpenRouter SDK:
   * - `string` — the common case for most models.
   * - `Array<ChatMessageContentItem>` — multi-part content; text
   *   parts are concatenated.
   * - `null | undefined | ""` — reasoning models (e.g. gpt-5-nano,
   *   o3-mini) may exhaust the `maxTokens` budget on internal
   *   reasoning tokens, leaving no room for visible content.
   *   The method detects this via `finishReason === "length"` and
   *   throws an actionable error.
   *
   * @throws {Error} If the response has no choices or no usable content.
   */
  private extractContent(response: ChatResponse): string {
    ...
    // ── Array of content items (multi-part) ──────────────────────────
    if (Array.isArray(content)) {
      ...
      // Falls through to the empty-content diagnostics below.
    }
    ...
    // ── Empty / null content — build an actionable error ─────────────
    const finishReason = firstChoice.finishReason;
    const hasReasoning =
      typeof firstChoice.message?.reasoning === "string" &&
      firstChoice.message.reasoning.length > 0;

    if (finishReason === "length") {
      const parts = [
        "OpenRouter response was truncated (finishReason: length) and produced no visible content.",
      ];
      if (hasReasoning) {
        parts.push(
          "The model used all available tokens for internal reasoning, leaving none for the actual response.",
        );
      }
      parts.push(
        "Increase maxTokens (reasoning models need budget for both reasoning and output tokens).",
      );
    ...
    if (finishReason === "content_filter") {
      throw new Error(
        "OpenRouter response was blocked by a content filter (finishReason: content_filter).",
      );
    }
    ...
    const detail = [
      ...
      `refusal: ${JSON.stringify(firstChoice.message?.refusal ?? null)}`,
    ].join(", ");

    throw new Error(
      `OpenRouter response contained no content (${detail}).`,
    );
  }
}
