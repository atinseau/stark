...
describe("Agent-Subtask mapping in InformationBroker", () => {
  it("findCandidateTargets prioritizes agents with dependencies on the source", async () => {
    ...
    const _capturedTargets: string[] = [];
    const mockConversations = {
      sendOneShotJson: mock((_role: any, _prompt: string) => {
  ...
  it("isAgentForSubtask returns true when mapping exists and false otherwise", async () => {
    ...
    const dependencies = [{ from: "s1", to: "s2", type: "blocking" as const }];
    ...
    const promptText = mockConversations.sendOneShotJson.mock
      .calls[0][1] as string;
...
describe("InformationBroker — full result sharing", () => {
  ...
  it("uses higher maxTokens when promptResultSummary is present", async () => {
    ...
    expect(capturedOptions.maxTokens).toBe(12288);
  });
});
