...
function validateSharingAnalysis(
  data: unknown,
): SharingAnalysisResult | null {
...
describe.skipIf(!HAS_API_KEY)(
  "Full result sharing — integration",
  () => {
    // ────────────────────────────────────────────────────────────
    ...
    describe("batched sharing with promptResultSummary", () => {
      it.concurrent(
        ...
        async () => {
          ...
          expect(decision.targetAgentId).toBe("agent-test-writer");
    ...
    describe("deduplication with enriched deltas", () => {
      it.concurrent(
        ...
        async () => {
          ...
          expect(info).toContain("jwt") || expect(info).toContain("token");
    ...
    describe("no sharing when irrelevant despite rich summary", () => {
      it.concurrent(
        ...
        async () => {
          ...
          const decision = result.decisions[0]!;
          expect(decision.targetAgentId).toBe("agent-cli-dev");
      ...
      it.concurrent(
        ...
        async () => {
          ...
          const testWriterDecision = result.decisions.find(
            (d) => d.targetAgentId === "agent-test-writer",
          );
          const cliDecision = result.decisions.find(
            (d) => d.targetAgentId === "agent-cli-dev",
          );

          expect(testWriterDecision).toBeDefined();
          expect(testWriterDecision!.shouldShare).toBe(true);
          expect(testWriterDecision!.information.length).toBeGreaterThan(30);

          expect(cliDecision).toBeDefined();
