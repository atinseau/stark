...
describe("InformationBroker — Sharing Deduplication", () => {
  ...
  function createBroker(overrides?: {
    ...
    const mockConversations = overrides?.mockConversations ?? ({
  ...
  function makeSharingDecision(overrides?: Partial<SharingDecision>): SharingDecision {
    return {
      shouldShare: true,
      ...
      information: overrides?.information ?? "Some useful information for the target agent",
    };
  }
  ...
  it("recordSharing respects the MAX_SHARING_RECORDS_PER_TARGET limit", () => {
    ...
    expect(allRecords[0]!.informationSummary).toBe("Information batch number 5");
    // The last one should be index 24
    expect(allRecords[19]!.informationSummary).toBe("Information batch number 24");
  });
  ...
  it("clearHistory clears all sharing history", () => {
    ...
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-A", information: "Info for A" }),
      DeltaType.PROMPT_COMPLETE,
    );
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-B", information: "Info for B" }),
      DeltaType.FILE_WRITTEN,
    );
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-C", information: "Info for C" }),
  ...
  it("LLM prompt includes sharing history for targets", async () => {
    ...
    const mockConversations = {
      sendOneShotJson: mock((role: any, prompt: string) => {
    ...
    expect(capturedPrompt).toContain("API has GET /users and POST /users endpoints");
  ...
  it("LLM prompt contains the non-redundancy criterion", async () => {
    ...
    const broker = new InformationBroker(
      ...
      [],
      silentLogger(),
      new Map([["t1", "agent-1"], ["t2", "agent-2"]]),
      new Map([["agent-1", "t1"], ["agent-2", "t2"]]),
    );
  ...
  it("records preserve different delta types correctly", () => {
    ...
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-X", information: "File content" }),
      DeltaType.FILE_WRITTEN,
    );
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-X", information: "Tool output" }),
      DeltaType.TOOL_COMPLETE,
    );
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-X", information: "Prompt result" }),
      DeltaType.PROMPT_COMPLETE,
    );
  ...
  it("clearHistory allows fresh recording after clearing", () => {
    ...
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-t", information: "Old info" }),
      DeltaType.FILE_WRITTEN,
    );
    expect(broker.totalRecordedSharings).toBe(1);
    ...
    broker.recordSharing(
      makeSharingDecision({ targetAgentId: "agent-t", information: "New info" }),
      DeltaType.PROMPT_COMPLETE,
    );
    expect(broker.totalRecordedSharings).toBe(1);
