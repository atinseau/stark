...
describe("parseToolCommand", () => {
  ...
  it("extracts complex shell commands", () => {
    expect(
      parseToolCommand({ command: 'docker info 2>&1 || echo "Docker not running"' }),
...
describe("parseToolOutput", () => {
  ...
  it("falls back to detailedContent when content is missing", () => {
    expect(
      parseToolOutput({ detailedContent: "detailed output\n<exited with exit code 1>" }),
  ...
  it("trims whitespace from result", () => {
    expect(parseToolOutput({ content: "  spaced  \n<exited with exit code 0>" })).toBe("spaced");
  });
  ...
  it("preserves output content before exit marker exactly", () => {
    const content = "PASS: all tests passed\nTotal: 42\n<exited with exit code 0>";
    expect(parseToolOutput({ content })).toBe("PASS: all tests passed\nTotal: 42");
  });
});
...
describe("parseExitCode", () => {
  ...
  it("handles large exit codes", () => {
    expect(
      parseExitCode({ content: "<exited with exit code 255>" }),
    ).toBe(255);
  });
});
