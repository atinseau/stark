...
} from "../src/utils/terminal-manager.ts";
 ...
 */
function req(params: Omit<CreateTerminalRequest, "sessionId">): CreateTerminalRequest {
...
describe("TerminalManager", () => {
  ...
  describe("create", () => {
    it("creates a terminal and returns a ManagedTerminal with all fields", () => {
      const terminal = manager.create(req({
        command: "echo",
        args: ["hello"],
        cwd: "/tmp",
      }));
  ...
  describe("release", () => {
    ...
    it("kills a running process on release", async () => {
      ...
      expect(
        result.exitCode !== 0 || result.signal !== null || result.signal !== undefined,
      ).toBe(true);
    });
  ...
  describe("edge cases", () => {
    ...
    it("handles rapid sequential creation and release", () => {
      for (let i = 0; i < 10; i++) {
        const terminal = manager.create(req({ command: "echo", args: [String(i)] }));
        manager.release(terminal.terminalId);
      }
